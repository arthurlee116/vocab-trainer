...
- **文档刷新**：README、AGENTS、CLAUDE 更新为 React 19 + GPT-5 mini，强调根 `.env`。 
- **VLM 识别**： `/api/vlm/extract` 使用 `openai/gpt-5-mini` 并通过 ProxyAgent 请求；README、AGENTS、CLAUDE 已同步 GPT-5 mini 说明，手动上传词表验证接口返回标准 `words`。 
...
| `server/` | Express API，`src/routes` REST 接口，`src/services` 对接 OpenRouter / GPT-5 mini，`src/db` SQLite。 |
