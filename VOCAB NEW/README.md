...
一个完全由 AI 实时驱动的词汇训练应用：上传单词表图片，GPT-5 读取词汇，Polaris Alpha 生成 3 大题型（共 60 题）的“超级 JSON”，并在练习结束后用中文输出 100 字左右的分析报告。游客模式使用浏览器 LocalStorage 保存历史，登录用户的记录则写入本地 SQLite 数据库。

### 核心特性

- ⚙️ **React + TypeScript SPA**：包含登录/注册、游客入口、Dashboard、题流练习、报告与历史等完整流程。
- 🧠 **VLM 识别**：服务端通过 `openai/gpt-5` 读取图片中的词汇，前端展示为标签，可手动增删。
...
- 前端：React 18（Vite）、React Router、Zustand、TypeScript、Axios
- 后端：Node.js + Express + TypeScript、SQLite（better-sqlite3 同步驱动）
- AI：OpenRouter `openai/gpt-5`（VLM）与 `openrouter/polaris-alpha`（题目与分析），使用 `response_format.json_schema` 获取结构化输出
...
复制仓库根目录的 `.env.example` 为 `.env`（可以放在根目录或 `server/` 内，后端会自动寻找）并填写：
...
| `POST /api/vlm/extract` | 调用 GPT-5 识别词表图片（body: `{ imageBase64 }`） |
...
- **VLM 提词**：`model: openai/gpt-5`，`response_format` 限制输出 `{ words: string[] }`
...
2. **安全**：若要在浏览器调用后端 API，请通过 `.env` 配置允许的 `CLIENT_ORIGIN`；生产环境不要在前端暴露 Operator Key。
