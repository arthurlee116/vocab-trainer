...
const UploadPage = () => {
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    try {
      ...
      setErrors([err?.response?.data?.message ?? '识别失败，请稍后再试']);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="page-section">
      <div className="panel">
        <h2>上传单词表图片</h2>
        <p className="sub">
          支持手写或打印照片，VLM (GPT-5) 会将识别结果转换为标签列表。您可以上传多张图片来识别更多词汇。
        </p>
            ...
            <div className="dropzone-content">
              {images.length === 0 ? (
                <>
                  <Upload size={48} />
                  <p>拖放或点击上传</p>
                  <p className="hint">支持 PNG/JPEG/WEBP/GIF 格式，最多 {MAX_FILES} 张图片</p>
                </>
              ) : (
                <p className="file-count">{images.length}/{MAX_FILES} 张图片已选择</p>
              )}
            </div>
          </div>
